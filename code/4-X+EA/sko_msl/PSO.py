...
class PSO(SkoBase):
    ...
    def __init__(self, func, n_dim=None, pop=40, max_iter=150, lb=-1e5, ub=1e5, w=0.8, c1=0.5, c2=0.5,
        ...
        self.X = chrom2x(crtbp(), self.lb, self.ub)
...
def crtbp():
    ...
    top_indices = sorted_indices[:36]
    top_strucs = [BO_struc_list[i] for i in top_indices]

    Chrom = top_strucs_list2arrfunc(top_strucs)

    return Chrom


def chrom2x(Chrom,lb_set,ub_set):
    X = np.array(lb_set) + (np.array(ub_set) - np.array(lb_set)) * Chrom
    return X